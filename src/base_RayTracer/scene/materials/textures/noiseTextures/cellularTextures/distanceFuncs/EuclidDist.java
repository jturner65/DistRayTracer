package base_RayTracer.scene.materials.textures.noiseTextures.cellularTextures.distanceFuncs;

import base_Math_Objects.vectorObjs.doubles.myPoint;
import base_RayTracer.scene.materials.textures.noiseTextures.cellularTextures.distanceFuncs.base.Base_DistFunc;

public class EuclidDist extends Base_DistFunc {
    @Override
    public double calcDist(myPoint v1, myPoint v2) {return v1.dist(v2);}    
    @Override
    public String toString(){return "Euclidean distance function";}
}//euclidDist